...
export class DashboardDetailsService {
  private dashboardDetailsUrl = 'http://home.loosescre.ws/~keith/synCare/server.php?command=users'
  ...
  getDashboardDetails(): Promise<Array<DashboardDetails>> {
    return this.http
      .get(this.dashboardDetailsUrl,{headers: this.callHeaders})
      .toPromise()
      .then((response) => {
        console.log(response);
        return response.json().dashboardDetails as DashboardDetails[];
      })
